...
def read_base_lines(base: str) -> tuple[list[str], Path | None]:
    """Read processed TXT lines for the base, stripping blanks. (Usada no modo Por Cena)"""
...
def split_into_sentences(text: str) -> list[str]:
    """Split raw text into sentences (Usada no modo Global)"""
    if not text:
        return []
...
def read_processed_sentences(base: str) -> tuple[list[str], Path | None]:
    """Reads raw text, splits into sentences, and returns the list (Usada no modo Global)"""
    txt_path = locate_processed_txt(base)
    if txt_path is None:
        return [], None
...
def group_lines(lines: list[str], group_size: int, joiner: str = "\n") -> list[str]:
    ...
    for i in range(0, len(lines), group_size):
        ...
        if not chunk_lines:
            continue
        # CORREÇÃO: Usa o 'joiner' para juntar as linhas (será '\n' para o modo Por Cena)
...
def parse_json_suggestions(raw: str) -> list[str]:
    ...
    try:
        ...
        pass
    # Fallback para parsing simples (lista não-JSON)
...
def list_ready_for_suggestions() -> list[str]:
    mf = load_manifest()
    # Usando 'txt' como done, como no seu código simples original, para consistência.
    return [
        base
...
def process_base_full_script(base: str, suggestion_count: int, chosen_profiles: list[str]) -> None:
    ...
    try:
        # Modo Global usa read_processed_sentences (quebra em sentenças)
        sentences, txt_path = read_processed_sentences(base)
        if txt_path is None:
        ...
        # Lógica de preenchimento/corte de sugestões
        ...
        # Divisão das sugestões entre os perfis
...
# ==========================
# PER-LINE MODE (Modo Por Cena - Corrigido)
# ==========================
def process_base(
    base: str,
    group_size: int,
    ...
    # CORREÇÃO: Usar a função de leitura linha-a-linha (read_base_lines)
    ...
    try:
        ...
        prompt_core = load_text(PROMPT_PATH)
        # CORREÇÃO: Prompt com a instrução clara de gerar UMA sugestão por BLOCO (cena)
        full_prompt = (
            f"{prompt_core}\n\n"
            f"--- TEXT ---\n\n"
        ...
        # CORREÇÃO: Usar '\n' como joiner para agrupar as falas, mantendo a estrutura original
        ...
        # Lógica de repetição/corte de cenas
        ...
        for prof_name, (start_idx, end_idx) in zip(chosen_profiles, ranges):
            ...
            with open(out_path, mode, encoding="utf-8") as f_out:
                for j, scene_idx in enumerate(
                    ...
                    # CORREÇÃO: Bloco de escrita com "Original:" e falas em linha única
                    block = [
                        f"Scene {scene_idx}",
                        "Original:",
                        scene_text.replace("\n", " "), # Junta as linhas originais (que vieram separadas por \n)
...
def main() -> None:
    try:
        ...
        for base in selected:
            # Usa count_sentences_for_base, que usa a lógica de sentenças para a contagem inicial
        ...
        for base in selected:
            if use_global_mode:
                ...
                # Chama o modo Por Cena corrigido
